...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = NXTChain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < NXTChain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số tiền giao dịch lớn hơn " + NXTChain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            NXTChain.UTXOs.remove(i.UTXO.id);
        }
